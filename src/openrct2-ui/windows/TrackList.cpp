...
public:
    ...
    void OnScrollDraw(const int32_t scrollIndex, rct_drawpixelinfo& dpi) override
    {
        ...
        else
        {
            ...
            {
                // Highlight
                const ScreenCoordsXY bottomRight{ width, screenCoords.y + SCROLLABLE_ROW_HEIGHT - 1 };
                gfx_filter_rect(&dpi, { screenCoords, bottomRight }, FilterPaletteID::PaletteDarken1);
        ...
        for (auto i : _filteredTrackIds)
        {
            if (screenCoords.y + SCROLLABLE_ROW_HEIGHT >= dpi.y && screenCoords.y < dpi.y + dpi.height)
            {
                ...
                {
                    // Highlight
                    ScreenCoordsXY bottomRight{ width, screenCoords.y + SCROLLABLE_ROW_HEIGHT - 1 };
                    gfx_filter_rect(&dpi, { screenCoords, bottomRight }, FilterPaletteID::PaletteDarken1);
                    stringId = STR_WINDOW_COLOUR_2_STRINGID;
                }
                else
